...
class MinecraftAdapter extends koishi_1.Adapter {
    ...
    serializeOutgoingMessage(message) {
        ...
        if (Array.isArray(message))
            return message.map(extractText);
    ...
    createSession(bot, type, payload) {
        ...
        switch (type) {
            case 'chat':
                ...
                if (payload == null) {
                    ...
                    messageText = payload.message.map((it) => (typeof it === 'string' ? it : it?.content ?? it?.text ?? JSON.stringify(it))).join(' ');
                }
        ...
        try {
            ...
            event.context.options = Object.assign({}, event.context.options || {}, { room: roomId });
            ...
            if (this.debug)
                logger.warn('[DEBUG] Failed to add compatibility room/context:', e);
        }
...
(function (MinecraftAdapter) {
    MinecraftAdapter.Config = koishi_1.Schema.object({
        ...
        tokenizeMode: koishi_1.Schema.union([koishi_1.Schema.const('split'), koishi_1.Schema.const('none')]).description('入站消息的分词模式：split=按空白分词（默认），none=不分词，保留原文').default('split'),
    });
